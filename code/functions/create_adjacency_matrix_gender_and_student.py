...
def create_ordered_dict(keys, value1, value2):
    return(OrderedDict([(keys[i], (value1[i], value2[i])) for i in range(len(keys))]))




def create_adj_membership_gender_student(graph, dictionary,  val_to_drop, delete_na_cols, diagonal, directed_type, attribute):
    keys = np.array(range(len(dictionary.keys())))      ## update keys
    y_vector = np.array([i[0] for i in dictionary.values()])#np.array(dictionary.values())            ## we relabel keys but this preserves corresponding value with updated keys
    year_vector = np.array([i[1] for i in dictionary.values()])
    ...
    if np.sum((y_vector==val_to_drop)+0) > 0: # first confirm there are >0 NA labeled nodes
        ...
        year_vector = year_vector[y_vector!=val_to_drop]
        ...
        attr_new = create_ordered_dict(range(len(y_vector)),
                                       y_vector,
                                       year_vector)
    ...
    if directed_type == None:
        if nx.number_connected_components(graph) > 1:
            ...
            dict_tmp = nx.get_node_attributes(largest_cc_subgraph, 'gender')
            y_vector = np.array([i[0] for i in dict_tmp.values()])
            year_vector = np.array([i[1] for i in dict_tmp.values()])
    ...
    if np.sum((np.sum(A_final,1)==0)+0)>0 and directed_type != 'in' and directed_type != 'out':
        ...
        year_vector = year_vector[np.array(subset_training_test_deg_0)]
        A_final = np.copy(A_cv) # only drop NA nodes as rows and NOT as columns

    return(np.array(y_vector), np.array(year_vector), np.matrix(A_final))
