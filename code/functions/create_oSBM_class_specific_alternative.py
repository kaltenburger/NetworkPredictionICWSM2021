    ...
    for i in range(len(class_size_vect)):
        for j in range(len(class_size_vect)):
            ...
            if i==j:
                ...
                if i<j:
                    expected_prob_matrix[idx:idx+class_size_vect[i],jdx:jdx+class_size_vect[j]] = out
    #print(bernoulli.rvs(expected_prob_matrix))
    A_ij_tmp = np.matrix(bernoulli.rvs(expected_prob_matrix)) #np.matrix(map(bernoulli.rvs,expected_prob_matrix))
    Adj_corrected = np.matrix(np.triu(A_ij_tmp, k=0) + np.transpose(np.triu(A_ij_tmp, k=1)))    
    Membership = np.concatenate(list(map(np.tile, np.array(range(len(class_size_vect))), class_size_vect)), 0)
