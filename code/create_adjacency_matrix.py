...
def create_adj_membership(graph, dictionary,  val_to_drop, delete_na_cols, diagonal, directed_type, attribute):
    ...
    y_vector = np.array(list(dictionary.values())) #np.array(dictionary.values())            ## we relabel keys but this preserves corresponding value with updated keys
    ...
    if directed_type == None:
        if nx.number_connected_components(graph) > 1:
            print('more than 1 component')
            max_cc = np.max(list(map(len,map(nx.nodes,nx.connected_component_subgraphs(graph, copy=True)))))
            subset_cc =  list(map(len,list(map(nx.nodes,nx.connected_component_subgraphs(graph, copy=True)))))==max_cc
            ...
            y_vector = np.array(list(nx.get_node_attributes(largest_cc_subgraph, attribute).values()))
            keys = np.array(range(len(np.array(list(nx.get_node_attributes(largest_cc_subgraph, attribute).keys())))))
